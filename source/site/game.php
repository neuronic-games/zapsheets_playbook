...
$_coverRaw = $game['Cover URL'] ?? ($game['Image URL'] ?? ($game['ImageURL'] ?? ($game['Image'] ?? '')));
if (preg_match('/^=IMAGE\("([^"]*)"\)$/i', $_coverRaw, $_cm)) { $coverG = $_cm[1]; } else { $coverG = $_coverRaw; }
$pageUrl   = $game['Page URL']      ?? '';
$sellUrl   = $game['Sellsheet URL'] ?? ($game['SellsheetURL'] ?? ($game['Sellsheet'] ?? ''));
$videoUrl  = $game['Video URL']     ?? ($game['VideoURL']     ?? ($game['Video']     ?? ''));
$playUrl   = $game['Play URL']      ?? ($game['PlayURL']      ?? ($game['Play']      ?? ''));
$rulesUrl  = $game['Rules URL']     ?? ($game['RulesURL']     ?? ($game['Rules']     ?? ''));
...
$bggMechanics  = [];
$bggCategories = [];
$_bggColUrl    = $game['BGG'] ?? ($game['BGG URL'] ?? ($game['BGGURL'] ?? ''));
$bggUrl        = $bggId ? 'https://boardgamegeek.com/boardgame/' . $bggId : $_bggColUrl;
if ($bgg) {
