<?php
/**
 * NoteBoard setup landing page.
 * Owner enters their Google Sheet ID/URL, runs setupNoteboard.php,
 * and sees the list of per-game feedback links.
 */

$_raw  = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
$_base = preg_replace('#/noteboard/?$#', '/', $_raw);
...
<title>NoteBoard – Set Up Feedback Collection</title>
...
.connect-btn:hover:not(:disabled) { background:#333; transform:translateY(-1px); box-shadow:0 4px 14px rgba(0,0,0,.22); }
...
(function() {
  ...
  var _pending = [];
  function scheduledLog(ms, msg, cls) { _pending.push(setTimeout(function() { addLog(msg, cls); }, ms)); }
  ...
  window.startSetup = function() {
    ...
    xhr.onload = function() {
      ...
      if (!result || result.error) {
        ...
        if (isPermErr) {
          addLog('✕  This sheet has not been shared with the NoteBoard service account.', 'err');
